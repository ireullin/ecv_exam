...
class DataBase(val call: ApplicationCall, val newSession:(correct:Int, total:Int)->HitSession){
    companion object{
    ...
    fun readSrc(){
        val lines = File("data/Database.txt").readLines(Charsets.UTF_8).map{it.trim()}.filter { it.isNotEmpty() }
        val subtitles = mutableListOf<MutableList<String>>()
//        val qNumReg = Regex("^\\d+")
        lines.forEach {line->
            println(line)
            when( line.startsWith("Question #") ) {
                true -> {
                    val cut = line.replace("Topic 1","<br>")
                    subtitles.add(mutableListOf(cut))
                }
        ...
        val _questions = subtitles.mapIndexed {i,sub->
            ...
            sub.forEach{ line->
                ...
                when{
                    ...
                    line.startsWith("Correct Answer:") -> part.add(mutableListOf(line.replace("Correct Answer:","").trim() ))
